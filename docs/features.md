...
Comments translate with the article rather than separately: they queue behind it, so each reply is translated against the glossary, protected terms, and subject domain the article established, and a reply renders a proper noun the same way as the passage it replies to. Requests follow what is on screen — the thread's first page, and the rest when it is revealed — so a discussion with hundreds of replies does not spend a model call, or a network request, on replies nobody has looked at.
...
| Article parser | legibility | Per-device preference; either parser can be chosen for one article from inside the reader. Both run on this device — legibility as WebAssembly in an offscreen web view with no network of its own | None beyond the article page the reader already fetches |
| Comments in the reader | On | Per-device toggle beside the parser; only legibility extracts a thread. Threads are stored in this device's local database, never in the sync folder, so an article first read on another device shows its body without its discussion | None beyond the article page the reader already fetches |
