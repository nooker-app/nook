...
/// twice a loop.
/// REVERSED, and that is the single measured fix in this pass with the highest payoff.
///
/// Read off a real render of the previous arrangement, perpendicular to each limb:
///
///   far arm    63px under the nest  ->  34px at the bottom edge   TAPER POINTS DOWN
///   main limb 102px under the nest  -> 100px at the bottom edge   near parallel
///
/// The far arm was drawn butt-up: its blunt end was buried under the nest and its NEEDLE POINT was
/// aimed at the bottom-left corner. A member that gets thinner as it descends is not carrying
/// anything — the eye reads it as hanging, or as the far end of a stick — so the nest was, again,
/// sitting on the end of something. That is the old "on a branch tip" rejection wearing the costume
/// of a fork, and it is why the crook measured better and still read wrong.
///
/// So the arm now ASCENDS: butt off-canvas at the lower left, needle tip buried under the nest.
/// Same silhouette family, same corner of the frame, opposite gradient — 81px where it crosses the
/// bottom edge, 63px where it goes under the nest's left fringe. Thicker below a thinner thing,
/// which is the whole of how support reads.
///
/// 29 degrees rather than the old 24: the two limbs measured 24.0 and 26.8 degrees from HORIZONTAL,
/// i.e. within 2.8 degrees of mirror symmetry, which is a tent whatever the thicknesses are. The
/// previous pass believed the asymmetry was carried by weight (52 against 102); at the size this is
/// viewed, a 2:1 thickness difference does not break a symmetric silhouette and a 5-degree slope
/// difference plus an opposite taper does.
///
/// The tip lands at local (2669, 1123) = world (2430, 1035), which is under the nest's FLOOR and
/// right of the hollow, so it is covered by the near wall and the near rim — both drawn after the
/// crook. Its butt is at world (760, 1960), 314px below the canvas, which is nine times the 36px
/// the sway can walk it.
const BOUGH2 = {at: {x: -805, y: 2805}, rot: -29, scale: 8.28, len: 419};
...
/// THE TRUNK, and it is the answer to "나무가 존재하여" — make the tree exist.
///
/// Measured on the previous frame: the top 400px of the canvas carried 0.00% ink, all the wood lived
/// below y 1100, and no member in the picture was within 60 degrees of vertical. Both limbs ran at
/// 24-27 degrees from HORIZONTAL and both left the frame going DOWN, so there was nothing under the
/// wood, nothing above it, and nothing pointing at the ground. The nest was the topmost solid object
/// in its own support structure. Anything that is the top of its own structure with both supports
/// slanting away beneath it is balanced, not seated, and no amount of tuning the two arms fixes that
/// — the missing element is not a better branch, it is a TRUNK.
///
/// Placement is solved against three hard edges rather than picked:
///
///   THE SUN. `SUN` is at (3310, 257) with a 480px core and the standing rule is that nothing is
///   ever drawn over it. The core spans x 2905..3715 where it crosses the top edge, so a trunk that
///   leaves through the top must leave LEFT of 2905. At rot -98 (8 degrees off vertical, leaning
///   left as it rises, which is also which way a tree leans away from an open side) the trunk spans
///   2614..2852 at y 0 and clears the core by 53px. Vertical would have clipped it by 34px, and
///   leaning it the other way runs straight through it.
///
///   THE NEST. Its ink ends at x 2730 and the safe box ends at 2742, so a trunk placed to stand
///   CLEAR of the nest has 12px to do it in — a tangent, which is the worst of both. It therefore
///   overlaps: at the nest's own height the trunk runs 2652..2902, i.e. 78px of it is behind the
///   nest's right fringe. The near wall and near rim are drawn after the wood, so the nest's own
///   strands cross in front of the bark and the two objects interlock instead of touching. That is
///   what "얹어두다" needs — the nest is set down INTO the angle between trunk and limb, not onto a
///   line.
///
///   THE SAFE BOX. 92px of trunk falls inside it (2650..2742), behind the nest, and that is
///   deliberate: the guaranteed view previously contained no wood at all except a 38px sliver of
///   the Λ's apex at its floor. It now contains the thing the wood comes from.
///
/// Thickness: 250px at the butt, against the main limb's 102. `baseHalf` clamps at 9, so this is
/// `stout: 1.5` to sit ON the clamp (any seed jitter is absorbed) and scale 25.25 to carry it — 274
/// units at 25.25 is 6918 local, 3805 world. Only the bottom half of that is ever on screen, and
/// with `needle` the width at the top edge is still 95% of the butt: a trunk should not visibly
/// taper across 1600px, and this one does not.
///
/// `sweep: 'stiff'`. Bowed puts up to 8.4% of length of chord deviation into it — 320px on this
/// limb — which at trunk width is a banana. Stiff still carries 2-10 degrees of bend, the kink and
/// the undulation, so the silhouette has knuckles and no straight edge, at trunk scale.
const TRUNK = {at: {x: 3031, y: 2751}, rot: -98, scale: 25.25, len: 274};
...
/// The trunk's centreline and its left flank, in scene-local units, so anything that has to be ON
/// the trunk is placed from the trunk rather than typed. `t` is distance from the butt.
const TRUNK_DIR = {x: Math.cos(TRUNK.rot * D2R), y: Math.sin(TRUNK.rot * D2R)};
const TRUNK_HALF = 9 * TRUNK.scale; // baseHalf is on the clamp, so this is exact at the butt
/// `off` is signed across the trunk; at rot -98 the normal (-sin, cos) points RIGHT, so the left
/// flank is at off = -TRUNK_HALF.
const trunkAt = (t: number, off = 0) => ({
  x: TRUNK.at.x + TRUNK_DIR.x * t - TRUNK_DIR.y * off,
  y: TRUNK.at.y + TRUNK_DIR.y * t + TRUNK_DIR.x * off,
});

/// WHERE THE LIMB ENDS. The main bough used to run from under the nest all the way to the bottom
/// right corner, 1368px of it, and once a trunk exists that run is a limb passing IN FRONT of its
/// own tree and coming out the other side — the same "spear through" defect the far arm was
/// rejected for, at four times the size.
///
/// So the bough is clipped to the half plane left of a line 90 local units INSIDE the trunk's left
/// flank. The cut edge is covered by trunk ink, so what the eye sees is a limb that goes BEHIND
/// the trunk and stops there: a junction nobody had to draw, which is the same trick that hides
/// the far arm's butt, applied to the other end of the wood.
///
/// 205 local units in, i.e. nearly to the centreline, and NOT the 90 tried first. At 90 the cut
/// landed inside the trunk's blurred edge, where the trunk is only partly opaque, and a render
/// showed exactly what that predicts: the limb's flat end cap standing as a straight dark diagonal
/// against a soft flank, with a pale wedge between them. A butt cut is hidden by ink or it is not
/// hidden. 205 puts it under 200+ local units of solid trunk.
///
/// The line follows the flank rather than being vertical: the trunk leans 8 degrees, and a vertical
/// cut would have sat 60 local units inside the flank at the nest's height and 60 outside it at the
/// canvas floor.
const BOUGH_CLIP = (() => {
  const inset = -(TRUNK_HALF - 205);
  const a = trunkAt(1300, inset);
  const b = trunkAt(500, inset);
  const dx = (b.x - a.x) / (b.y - a.y);
  const at = (y: number) => a.x + dx * (y - a.y);
  return `M -6000 -6000 L ${at(-6000).toFixed(1)} -6000 L ${at(8000).toFixed(1)} 8000 L -6000 8000 Z`;
})();
...
/// BARK ON THE TRUNK, and it is not optional. Rendered as one smooth cross-thickness ramp a 250px
/// column reads as a stocking or a leather boot — the same "polished strap" defect the main limb's
/// grain was invented for, at two and a half times the width, and worse because a trunk is the one
/// piece of wood in the picture whose surface the eye expects to be able to read.
///
/// Seven rods rather than the limb's five, and they run 24-48% of the trunk's length rather than
/// 15-45%: bark furrows on a trunk are long and near-parallel, and short marks on a wide column
/// read as scratches.
///
/// ALL SEVEN ARE DARK, and that is a correction made from a render rather than a preference. The
/// limb's grain uses the nest ramp's pale rungs at 0.3-0.5 alpha, which works at 100px wide because
/// the marks are small; laid on a 250px column and then blurred, the same rungs came out as pale
/// straw-coloured blades and the trunk rendered as a bundle of bamboo. A furrow is a groove — it is
/// the absence of light, never a highlight — so these are rungs 0-1 at 0.10-0.18, and what lights
/// the trunk is the `cbBark` ramp underneath them and nothing else.
const TRUNK_GRAIN = [
  {seed: 0x6a1, s: 0.06, off: -5.8, len: 110, rot: 0.9, rung: 0, op: 0.26},
  {seed: 0x6b2, s: 0.1, off: -1.4, len: 130, rot: -0.7, rung: 1, op: 0.2},
  {seed: 0x6c3, s: 0.2, off: 3.2, len: 120, rot: 1.3, rung: 0, op: 0.2},
  {seed: 0x6d4, s: 0.3, off: 6.4, len: 100, rot: -1.1, rung: 1, op: 0.16},
  {seed: 0x6e5, s: 0.38, off: -4.2, len: 90, rot: 1.7, rung: 0, op: 0.26},
  {seed: 0x6f6, s: 0.5, off: 0.8, len: 80, rot: -1.4, rung: 1, op: 0.2},
  {seed: 0x6a7, s: 0.62, off: -6.6, len: 70, rot: 0.6, rung: 0, op: 0.22},
] as const;

/// A LIMB OFF THE TRUNK, low and falling away to the right, and it is doing two jobs.
///
/// Cutting the main bough at the trunk emptied the bottom right sixth of the frame — 850x550px of
/// bare paper on the sun side, which is the brightest quarter of the picture and therefore the one
/// place a hole shows. This fills it with the thing that ought to be there anyway.
///
/// And it is what makes the trunk a TREE rather than a post: one limb leaving it is an accident,
/// two leaving it on opposite sides at different heights is a habit of growth. Its butt is buried
/// inside the trunk (drawn before it, same trick as everything else here) and it TAPERS AWAY from
/// the trunk — 108px where it leaves, 74px at the canvas edge — which is the direction the far arm
/// had backwards and the whole reason this pass exists.
///
/// 34 degrees below horizontal, against the main bough's 27 and the ascending arm's 29: three
/// members within 7 degrees of each other is a fan, and the eye reads a fan as one splayed object.
const LIMB3 = {t: 1000, off: 60, rot: 34, scale: 9.1, len: 250};
const LIMB3_GEOM = buildTwig({
  seed: 0x3311,
  length: LIMB3.len,
  arch: 'bare',
  sweep: 'bowed',
  ...
  maxDepth: 0,
  stout: 1.4,
  lightLocalDeg: LIGHT_WORLD_DEG - LIMB3.rot,
  allowHighlight: false,
});
const LIMB3_AT = trunkAt(LIMB3.t, LIMB3.off);
const LIMB3_XF = `translate(${LIMB3_AT.x.toFixed(1)} ${LIMB3_AT.y.toFixed(1)}) rotate(${LIMB3.rot}) scale(${LIMB3.scale})`;
...
///
/// MOVED ONTO THE TRUNK. The bough is now cut where it enters the trunk (see BOUGH_CLIP), so local
/// x 3300 and 4100 — the two positions these were solved at — are past the cut and would have left
/// two twigs floating in the sky. They sprout from the trunk's left flank instead, which is worth
/// more than it costs: a bare column is a post, and a column with shoots off it is a tree. They
/// also do the job real nest sites do — smaller twigs rising AROUND the cup — which is half of why
/// a nest in a photograph reads as held rather than balanced.
///
/// Butts are inset 60 local units inside the trunk's left edge so the flat end cap is buried, and
/// they are drawn inside the trunk's own blur group, because a sharp twig growing out of a soft
/// trunk is two objects at two distances pretending to be one.
/// They HUG THE TRUNK — 258 and -74 degrees, i.e. within 20 degrees of the trunk's own axis — and
/// that is a constraint, not a look. Aimed out at the 205 degrees they used on the limb, a shoot
/// this size reaches screen (2199, 666): inside the safe box, above the nest, in open paper and in
/// the bird's lane. The prior pass has a note about exactly that shape ("a flat near-black
/// three-pronged claw in open paper") and it is the reason the old sprigs were all banished right
/// of screen 2900. Along the trunk they stay within 50px of its flank the whole way up.
/// BOTH ON THE RIGHT FLANK, which is a correction from a render. A shoot on the LEFT flank has two
/// places it can go and both are bad: aimed away from the trunk it lands above the nest inside the
/// safe box and inside the bird's lane, and aimed along the trunk — 258 degrees, against the trunk's
/// own 262 — it lies parallel to the flank it grew from and renders as a pale strip with a straight
/// edge down the trunk's left side, i.e. a split or a peeling. That was on screen at frame 60 and it
/// is why there are two of these and not three.
///
/// -74 and -58: shallow enough that the perpendicular run clears the trunk's own 227-unit half
/// width. At the 258 the left one used, the outward component was 0.07 of the length and the shoot
/// never left the silhouette at all.
const SPRIGS = [
  {seed: 0xc19, t: 2360, side: 1, rot: -74, len: 240, scale: 1.6},
  {seed: 0x84d, t: 1450, side: 1, rot: -58, len: 210, scale: 1.5},
] as const;
...
export const ProbeC: React.FC<{guides?: boolean}> = ({guides = false}) => {
  ...
  const sprigs = React.useMemo(
    () =>
      SPRIGS.map((s) => ({
        ...s,
        ...trunkAt(s.t, s.side * (TRUNK_HALF - 60)),
        geom: buildTwig({
          seed: s.seed,
          length: s.len,
          arch: 'forked',
    ...
    <AbsoluteFill style={{background: '#F3E3C4'}}>
      <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`}>
        <defs>
          ...
          {/* AERIAL PERSPECTIVE ON THE TRUNK, and it is fixing a timing defect as well as a tonal
              one. Local units, because it is used inside the scene group: local y -900 is 60px above
              the canvas top and local y 900 is world y 1445.

              The tonal half: the trunk ran at the same value from the canvas floor to the canvas
              ceiling, which is what a flat cutout does and not what 1600px of air does. #EEDCBB is
              the sky measured at (2400, 100).

              The timing half, which is why the top stop is as strong as 0.62: the bird crosses the
              trunk's column at about frame 148, and its ink is #2C170B against a trunk that measured
              luma 35-60 up there. A dark bird in front of dark wood is an invisible bird for three
              frames, twice — the loop plays it as a flicker. Hazed out, the wood at the bird's
              crossing height sits near luma 150 and the silhouette survives it. */}
          <linearGradient
            id="cbTrunkFade"
            x1="0"
            y1={-1000}
            ...
            gradientUnits="userSpaceOnUse"
          >
            <stop offset="0%" stopColor="#EEDCBB" stopOpacity={0.6} />
            <stop offset="25%" stopColor="#EAD6B4" stopOpacity={0.45} />
            <stop offset="60%" stopColor="#E0C8A0" stopOpacity={0.22} />
            <stop offset="100%" stopColor="#D8BB8E" stopOpacity={0} />
          </linearGradient>
          <filter id="cbAmbient" filterUnits="userSpaceOnUse" x={1500} y={1150} width={1500} height={450}>
            <feGaussianBlur stdDeviation="26" />
          </filter>
          {/* The trunk sits one plane BEHIND the nest, so it is one plane out of focus. Local units,
              because it is filtered inside the scene group: 13 local is 7px on screen. Enough to
              stop a 250px column of bark competing with the weave for the eye, not enough to turn
              it into the "wrinkled bedsheet" murk the left of the frame was once rejected for. */}
          <filter
            id="cbTrunk"
          ...
          {/* The half plane left of the trunk's inner line — where the main limb stops. */}
          <clipPath id="cbLimbEnd">
            <path d={BOUGH_CLIP} />
          </clipPath>
          <clipPath id="cbTrunkWood">
        ...
        <g transform={`rotate(${f4(rigid)} ${RIGID_PIVOT.x} ${RIGID_PIVOT.y})`}>
          {/* 8b THE TRUNK, first and therefore behind everything: the nest's under-shape, the cup,
              both rims and the main limb all draw over it, so it is the thing the tree is made of
              and never the subject. It carries `cbBark` for the same reason the far arm does —
              `userSpaceOnUse` on y -9.4..9.4 resolves in the path's OWN frame, so the cross-thickness
              ramp lies across the trunk and turns with it — with a dark overlay on top, because wood
              a plane further back and turned away from a sun that is behind it is a stop down the
              same material, not a different one. */}
          <g filter="url(#cbTrunk)">
            {/* Shoots and the low limb FIRST, so their butt cuts are under the bark they grow out
                of — the same order that hides the far arm's butt under the main limb. */}
            {sprigs.map((s) => (
              <g
                key={s.seed}
                transform={`translate(${s.x.toFixed(1)} ${s.y.toFixed(1)}) rotate(${s.rot}) scale(${s.scale})`}
              >
                {/* Bark, not straw. These carried `gradId(1, ...)` — the nest strands' own pale
                    ramp — when they lived on the limb at a twentieth of this size, and at trunk
                    scale that put two pale blades across the bark that read as fallen straw. */}
                <path d={s.geom.body} fill="url(#cbBarkFlip)" fillRule="nonzero" />
                <path d={s.geom.body} fill="#2A1A0A" fillRule="nonzero" opacity={0.22} />
              </g>
            ))}
            <g transform={LIMB3_XF}>
              <path d={LIMB3_GEOM.body} fill="url(#cbBark)" fillRule="nonzero" />
              <path d={LIMB3_GEOM.body} fill="#2A1A0A" fillRule="nonzero" opacity={0.22} />
            </g>
            <g transform={TRUNK_XF}>
              {/* cbBarkFlip, not cbBark, and this is measured rather than taste. The ramp is
                  `userSpaceOnUse` on the path's own y, light at -9.4; SVG's rotate(a) sends +y to
                  (-sin a, cos a), so at rot -98 the path's +y points RIGHT and the light lands on
                  the LEFT flank — the side away from a sun at (3310, 257). Measured on a render, the
                  trunk came out luma 73 on its shadow side and 30 on the side facing the sun, which
                  put maximum contrast exactly where the wood meets the brightest sky in the picture
                  and turned a 250px column into a black cutout. The flipped ramp is the same six
                  stops read the other way.

                  0.22 of the darkening overlay and not 0.34, for the same reason the aerial haze
                  exists: this wood is a plane further off than the limb, and things further off get
                  paler, not darker. */}
              <path d={TRUNK_GEOM.body} fill="url(#cbBarkFlip)" fillRule="nonzero" />
              <path d={TRUNK_GEOM.body} fill="#2A1A0A" fillRule="nonzero" opacity={0.22} />
            </g>
            {/* Furrows, clipped to the trunk so they can only ever be marks on its surface. */}
            <g clipPath="url(#cbTrunkWood)">
              <g transform={TRUNK_XF}>
                {trunkGrain.map((g) => (
                  <g
                    key={g.seed}
            ...
            {/* Air between the viewer and the top of the trunk. Last inside the trunk group, so it
                lies over the bark, the furrows and the shoots alike.
                A RECT CLIPPED TO THE TRUNK, and not the trunk path filled with the gradient, for the
                reason the file already records about cbBark: `userSpaceOnUse` resolves in the
                referencing element's OWN space, and the trunk path lives inside TRUNK_XF where its
                whole thickness is y -9..9. Filling the path directly put every pixel of it at the
                same offset in a ramp meant to span the canvas, and the render came back as one flat
                white wash over the entire column. The rect is in scene space, where the ramp means
                what it says. */}
            <g clipPath="url(#cbTrunkWood)">
              <rect x={1800} y={-2200} width={2400} height={5000} fill="url(#cbTrunkFade)" />
            </g>
          </g>

          {/* THE CROTCH SHADOW: the limb's own shade thrown onto the trunk it leaves, clipped to the
              trunk so it can only ever be a mark on that surface. A junction reads as a junction
              when the two pieces exchange light; without it the limb's cut end is merely covered,
              which stops it looking broken but does not make it look attached.

              Filtered at cbContact's 26 rather than left to the trunk group's own 6: unblurred and
              at ry 132 it rendered as a dark band wrapping the column — a rubber ring, not a shade.
              It sits just under the crossing and is angled along the limb. */}
          <g clipPath="url(#cbTrunkWood)">
          ...
          {/* 12 the sprigs used to be drawn here, on the bough. They are on the trunk now — see 8b. */}
          ...
          {/* The main limb, and everything painted on it, cut where it enters the trunk. */}
          <g clipPath="url(#cbLimbEnd)">
