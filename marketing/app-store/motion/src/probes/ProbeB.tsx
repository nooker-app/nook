...
/// THE BOUGH — rebuilt, because the previous pair did not carry the nest and the measurements say
/// exactly why. Two limbs left the frame going DOWN on either side of a junction hidden under the
/// weave, and the numbers off the render were these:
///
///   the wood's top edge under the nest ran 1179 at x2000, 1139 at x2100, 1104 at x2200, then a
///   maximum near x2370, then 1123 at x2600, 1163 at x2700, 1202 at x2800. That is a RIDGE, and its
///   crest fell within 10px of the nest's own ink centroid (2378.5). Slope 0.35 away to the left,
///   0.40 away to the right. A load balanced on a crest with the ground falling 20 degrees both ways
///   is a see-saw, and the eye reads see-saws before it reads branches;
///
///   at x2100 the nest's lowest ink was y1030 and the wood's top edge was y1139 — 109px of open sky
///   between them. Over x2027..2230, 29% of the nest's width, the nest touched no wood at all: it
///   overhung the crest. `seatTop` could not close that gap because where the main limb had already
///   ended, `seatTrim` (world y 1073) governs and the far arm was 66px below it;
///
///   the two arms measured 52px and 73px thick where they met. A fork whose arms are 0.7 of each
///   other has no parent: it is two sticks crossing. Neither continued past the junction, both
///   thinned away from it, and both left the canvas through the bottom edge;
///
///   and NOTHING IN THE FRAME ROSE. Every wood edge in the picture had a downward gradient away from
///   the crest at (2370, 1040). There was no near-vertical edge anywhere and no wood above the
///   nest's floor, so the wood had no origin, no implied mass, and nothing for the weight to go into.
///
/// What replaces it is one heavy limb whose run under the nest is LEVEL, entering from off-frame
/// right where an unseen trunk is, with a limb dropping off it on the right and a cradle of smaller
/// wood rising around the nest on the left. Solved rather than picked, and measured on its own
/// outline in world pixels:
///
///   top edge  1044 at x2100, 1049 at 2200, 1054 at 2300, 1059 at 2400, 1056 at 2500, 1054 at 2600,
///             1051 at 2700 — 15px of variation across the whole 715px the nest occupies, against
///             112px of ridge before. The seat is level to 1.2 degrees;
///   thickness  104px under the nest against 57-73px before — 0.32 of the nest's 327px height,
///             where it was 0.20. NOT more, and the ceiling is the generator rather than taste:
///             `baseHalf` clamps at 9 units, so world thickness is 0.049 x world length, and a
///             thicker limb is a LONGER limb drawn at a bigger scale. `buildTwig` caps its trunk at
///             5 links x 3 samples, so the outline is ~14 straight segments however long it is. At
///             the scale 16.5 that a 132px limb needs, that is a 240px straight run under the load,
///             and a 240px straight HORIZONTAL run reads as the edge of a table. It was rendered and
///             it did. 11.2 puts the segments back to 154px, which is where the old diagonal was;
///   right edge it crosses x3840 at y1129 with 156px of thickness — the mass leaves the frame
///             through the SIDE, not the bottom, which is what puts a trunk off-frame beside us
///             instead of a stick standing in mid-air.
///
/// seed 0x6a61 was chosen by a SPUR TEST and not by eye. `buildTwig` sometimes puts a hard elbow
/// where the outer flank overhangs, and on a diagonal limb nobody reads that; on a level one it hangs
/// off the UNDERSIDE in open paper. Seed 0x136d6, which this was built on for two rounds, grew a 39px
/// x 130px dark blade under world x2360 — directly beneath the nest's centre, and it took five
/// bisecting renders to prove it belonged to the limb and not to a twig. The search now scans
/// thickness every 10px along the visible run and rejects any placement whose maximum exceeds 1.42x
/// its own median. 0x6a61 peaks at 130 against a median of 108, and that peak is at x3530 — in the
/// bleed, on the far side of the drop.
const BOUGH = {at: {x: 5129, y: 1413}, rot: 186, scale: 11.8, len: 349};
...
/// THE CROTCH ARM — the limb the nest is built INTO, and the second half of the answer.
///
/// A nest on top of a line is a nest balanced on a line however level the line is. Real ones are
/// built where a limb DIVIDES, braced between two pieces of wood that go different ways, and that is
/// what this is: it leaves the bough's inside at world (2050, 1085), under the nest's left end, and
/// rises to the right at 17 degrees — so the two limbs open into a V with the nest down in it.
/// Level thing below, rising thing behind: the weight has somewhere to go in both directions.
///
/// It is not the old far arm turned round. That one FELL AWAY from the junction on the far side, so
/// the pair made a Λ with the nest on the apex and every wood edge in the frame pointing down. This
/// one rises, and it rises BEHIND the weave — drawn before the nest's under-shape, so the nest is
/// bodily in front of it and only its two ends show: a sliver in the 44px gap at the nest's left,
/// and then 560px of limb climbing away past the nest's right end into the bleed. A nest that hides
/// the middle of its own support is a nest that is sitting in something.
///
/// 66px thick against the bough's 132 — exactly half, so which one is the parent is never in
/// question. That costs 1300px of length (`baseHalf` clamps at 9 units, so world thickness is fixed
/// at 0.049 x world length), which puts its end at (3293, 705), 550px into the bleed. `arch:
/// 'forked'` with maxDepth 1 rather than 'bare' for that end alone: at this scale the generator's
/// own shoots come out 300-500px long, which is a branchlet and not the thorn a shoot proportioned
/// to a short twig would be — and it means the limb ENDS IN TWIGS instead of in the needle point
/// that got an earlier version rejected as a dart.
const BOUGH2 = {at: {x: 1540, y: 1278}, rot: -22, scale: 5.6, len: 349};
...
/// THE DROP. One more child, out at world x3150 where there is room for a junction to be seen, and
/// it earns its place twice. A bough that only ever leaves the frame through one edge has one
/// anchor; this gives the wood a second contact with the frame and 610px of the mass back that the
/// old diagonal used to put in the bottom-right corner. And it breaks the limb's UNDERSIDE, which
/// after the seat was levelled was a 1400px straight line — the same 14-segment outline problem the
/// knots answer on the top edge.
///
/// 59px against the bough's 104 at that column: 0.57, thinner, hanging off it, drawn before it so
/// the butt cap is covered. It leaves the canvas 553px below the bottom edge, which is 15 times the
/// 36px the sway can walk it.
const BOUGH3 = {at: {x: 3540, y: 1314}, rot: 62, scale: 6.25, len: 349};
const BOUGH3_GEOM = buildTwig({
  seed: 0x3719,
  length: BOUGH3.len,
  arch: 'bare',
  sweep: 'bowed',
  ...
  tip: 'needle',
  maxDepth: 0,
  lightLocalDeg: LIGHT_WORLD_DEG - BOUGH3.rot,
  allowHighlight: false,
});
const BOUGH3_XF = `translate(${BOUGH3.at.x} ${BOUGH3.at.y}) rotate(${BOUGH3.rot}) scale(${BOUGH3.scale})`;
...
/// The top of the wood a thing SITTING ON the bough should be anchored to. It is `boughTopAt` and
/// nothing else now, and that is a correction rather than a simplification: this used to be
/// `min(boughTopAt, bough2TopAt)` because the far arm was the wood under the nest's left half. The
/// crotch arm that replaced it RISES and passes behind the weave, so its top edge over the nest's
/// columns is ~100px ABOVE the bough — and a lashing anchored to the minimum was placed inside the
/// nest with its shoot hanging below the limb as a dark spike. Anything that ties the nest to the
/// wood ties it to the wood it rests on.
const woodTopAt = (x: number) => boughTopAt(x);
...
const GRAIN = [
  {seed: 0x5a1, s: 0.16, off: -5.2, len: 96, rot: 1.4, rung: 4, op: 0.5},
  {seed: 0x5b2, s: 0.24, off: 2.6, len: 84, rot: -1.0, rung: 0, op: 0.45},
  {seed: 0x5c3, s: 0.3, off: -1.4, len: 70, rot: 1.9, rung: 5, op: 0.34},
  {seed: 0x5d4, s: 0.37, off: 5.4, len: 76, rot: -1.6, rung: 0, op: 0.42},
  {seed: 0x5e5, s: 0.44, off: -3.4, len: 64, rot: 1.1, rung: 4, op: 0.46},
  {seed: 0x5f6, s: 0.5, off: 1.2, len: 58, rot: -2.2, rung: 5, op: 0.3},
  {seed: 0x607, s: 0.56, off: -5.8, len: 52, rot: 0.8, rung: 3, op: 0.44},
  {seed: 0x618, s: 0.62, off: 3.8, len: 46, rot: 2.0, rung: 0, op: 0.4},
  {seed: 0x629, s: 0.7, off: -2.2, len: 40, rot: -1.3, rung: 4, op: 0.38},
] as const;
...
/// `rot` is negative — the twigs RISE off the bark. On the old diagonal they lay along it at 202 and
/// 176 degrees and showed; on a level limb the same angles put both of them INSIDE the silhouette,
/// where they rendered as nothing at all. Rendered, measured, turned over.
const SPRIGS = [
  {seed: 0x711, x: 3300, rot: -29, len: 280, scale: 1.5},
  {seed: 0xc19, x: 4100, rot: -14, len: 240, scale: 1.3},
] as const;

/// THE CRADLE — small wood rising from the bough AROUND the nest, and the mark that turns a seat
/// into a basket. Measured on the old frame there was no wood anywhere above the nest's floor and no
/// near-vertical edge in the picture at all; every one of these is vertical-ish and every one of them
/// starts inside the bough and ends above the weave.
///
/// They are placed at the nest's two ENDS and nowhere else, for a reason that is compositional
/// rather than botanical: a riser standing in open sky right of the nest is a pole, and the safe box
/// ends 12px past the nest's right edge, so there is no open sky inside the guaranteed view to stand
/// one in. Every riser here leans so that its top lands OVER the weave, where it reads as a twig the
/// nest was built around instead of a stick beside it.
///
/// `x` is world-solved and stated in local units; `dy` puts each butt 90-150 units under the bough's
/// own top edge — inside the wood, not on it — so the bough covers the flat end cap. `back` ones are
/// drawn before the nest's under-shape and are hidden by the whole weave except at their tips;
/// `front` ones are drawn with the lashings, over the bark and under the near rim, so there is wood
/// on the near side of the nest as well as the far one. Wood on more than one side is the whole
/// difference between set down on and built into.
///
/// Tops are capped at world y 790 — the nest's own crown — because the sky above that belongs to the
/// bird, whose lowest pass over these columns is y 719.
const CRADLE_BACK = [
  {seed: 0x811, x: 1576, rot: -66, len: 320, scale: 2.15, dy: 90},
  {seed: 0x822, x: 1740, rot: -81, len: 300, scale: 2.0, dy: 80},
  {seed: 0x866, x: 1900, rot: -70, len: 270, scale: 1.8, dy: 80},
  {seed: 0x833, x: 2576, rot: -104, len: 300, scale: 1.9, dy: 80},
  {seed: 0x844, x: 2794, rot: -96, len: 320, scale: 2.0, dy: 85},
  {seed: 0x855, x: 3031, rot: -113, len: 280, scale: 1.75, dy: 75},
  {seed: 0x8bb, x: 3210, rot: -88, len: 260, scale: 1.7, dy: 70},
] as const;

const CRADLE_FRONT = [
  {seed: 0x877, x: 1685, rot: -73, len: 260, scale: 1.65, dy: 70},
  {seed: 0x899, x: 1830, rot: -86, len: 230, scale: 1.5, dy: 65},
  {seed: 0x888, x: 2540, rot: -97, len: 260, scale: 1.65, dy: 70},
  {seed: 0x8aa, x: 2700, rot: -108, len: 240, scale: 1.55, dy: 65},
] as const;

/// THE BRANCH END. The limb has to stop somewhere inside the frame — it cannot leave through the
/// left edge, and the arithmetic is the reason rather than the composition: world thickness is
/// 0.049 x world length, so a limb long enough to cross the frame and exit is a limb 190px thick,
/// and one thin enough to read as a continuation is 700px too short to get there. Both were solved
/// for and neither exists.
///
/// So it ends the way wood ends: it breaks up. Five twigs from the last 300 units of the limb,
/// fanned left and DOWN-LEFT rather than up — down, because up is where the wordmark is (ink to
/// x1983, y786..962) and a spray reaching for it crowds the type, while a drooping one falls into
/// the empty quarter under it. Their butts are inside the wood and the limb is drawn over them, so
/// what is left is a limb that frays instead of a needle point in open paper, which is the defect
/// an earlier version of this asset was rejected for by name.
///
/// The rotations are 146-177 and NOT 190-226. Both were rendered: the second set is what "fan it
/// downwards" produces if you forget that screen y grows down, and it aimed the whole spray at the
/// underside of the K. Measured, its highest shoot reached y966 against a wordmark whose ink ends at
/// 962. Below 180 is down.
const ENDSPRAY = [
  {seed: 0xa11, x: 1250, rot: 168, len: 300, scale: 1.95, dy: 62},
  {seed: 0xa22, x: 1320, rot: 154, len: 300, scale: 1.9, dy: 70},
  {seed: 0xa33, x: 1400, rot: 177, len: 240, scale: 1.45, dy: 74},
  {seed: 0xa44, x: 1470, rot: 146, len: 320, scale: 2.0, dy: 80},
  {seed: 0xa55, x: 1560, rot: 162, len: 260, scale: 1.6, dy: 76},
] as const;

/// KNOTS — stubs of broken-off wood on the bough's top edge, drawn BEFORE the bough so only the part
/// that clears its silhouette shows and the butt is covered. `buildTwig` caps its trunk at 5 links
/// with at most 3 samples each, so the outline is about 14 straight segments however long the limb
/// is. At this scale that is a 154px straight run, and on a level limb a straight run reads as a
/// machined edge. Four stubs put four corners into it that a ruler cannot be laid along.
const KNOTS = [
  {seed: 0x911, x: 1413, rot: -120, len: 150, scale: 1.2, dy: 80},
  {seed: 0x922, x: 1994, rot: -72, len: 130, scale: 1.15, dy: 75},
  {seed: 0x933, x: 3085, rot: -98, len: 150, scale: 1.25, dy: 80},
  {seed: 0x944, x: 4000, rot: -64, len: 140, scale: 1.15, dy: 75},
] as const;
...
///
/// `dy` is 74-88 and not 95-135, and the reason is a defect that only showed once the limb was
/// levelled. A lashing is `arch: 'single'`, so it carries one shoot, and the shoot's direction is
/// the generator's business rather than mine; on the old diagonal a downward one stayed inside a
/// limb whose vertical extent at any column was 1.5x its thickness, and on a level limb it came out
/// the UNDERSIDE as a 150px dark spike hanging in open paper at world x2380. Measured on the render
/// twice before it was found. Starting the butts 40-48 world px under a 104px top edge keeps the
/// whole twig, shoot included, inside the silhouette or above it.
///
...
const LASHINGS = [
  {seed: 0xaa1, x: 1730, rot: -66, len: 260, dy: 74},
  {seed: 0xee5, x: 1900, rot: -74, len: 300, dy: 80},
  {seed: 0xbb2, x: 2080, rot: -60, len: 320, dy: 88},
  {seed: 0xcc3, x: 2340, rot: -68, len: 300, dy: 88},
  {seed: 0xdd4, x: 2560, rot: -57, len: 260, dy: 82},
] as const;
...
export const ProbeB: React.FC<{guides?: boolean}> = ({guides = false}) => {
  ...
  /// Cradle wood and knots share one builder: same anchoring rule (butt inside the bough by `dy`),
  /// same 'brushy' end so nothing terminates in a point, different draw slots.
  const twiggy = React.useCallback(
    (
      list: readonly {seed: number; x: number; rot: number; len: number; scale: number; dy: number}[],
      arch: Arch,
      depth: number
    ) =>
      list.map((s) => ({
        ...s,
        y: boughTopAt(s.x) + s.dy,
        geom: buildTwig({
          seed: s.seed,
          length: s.len,
          arch,
  ...
  const cradleBack = React.useMemo(() => twiggy(CRADLE_BACK, 'single', 1), [twiggy]);
  const endspray = React.useMemo(() => twiggy(ENDSPRAY, 'forked', 2), [twiggy]);
  /// 'bare' and not 'single': these are the only twigs drawn IN FRONT of the bough, so a shoot
  /// pointing down leaves the silhouette on the underside with nothing to cover it. One did, as a
  /// 180px dark spike hanging off the limb at world x2320.
  const cradleFront = React.useMemo(() => twiggy(CRADLE_FRONT, 'bare', 0), [twiggy]);
  /// 'bare': a knot is a stub, and a shoot on one of these poked out UNDER the bough's lower edge —
  /// where nothing covers it — as a dark spike hanging off the limb. Rendered, measured, removed.
  const knots = React.useMemo(() => twiggy(KNOTS, 'bare', 0), [twiggy]);
      ...
      <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`}>
        <defs>
          ...
          <linearGradient
            ...
            spreadMethod="reflect"
          >
            <stop offset="0%" stopColor="#9C7039" />
            <stop offset="7%" stopColor="#875F33" />
            <stop offset="18%" stopColor="#674726" />
            <stop offset="34%" stopColor="#563A1D" />
            <stop offset="54%" stopColor="#472E16" />
            <stop offset="78%" stopColor="#382310" />
          ...
          {/* Grain rods are `buildTwig` outlines, so they are POLYGONS: at bough scale each one
              rendered as a flat plate with straight edges and corners, and four of them made the
              limb look like folded card. Blurred they are what they were always meant to be — a
              tonal streak along the wood — and the clip on the same group runs after the filter, so
              the softening cannot spill onto bare paper. */}
          <filter id="cbGrainSoft" filterUnits="userSpaceOnUse" x={700} y={900} width={4600} height={800}>
        ...
        <g transform={`rotate(${f4(rigid)} ${RIGID_PIVOT.x} ${RIGID_PIVOT.y})`}>
          ...
          {/* 8b THE CROTCH ARM AND THE BACK CRADLE, BEHIND THE WHOLE NEST. Draw order is the
              argument here: everything in this block is covered by the weave and uncovered by the
              bough, so each piece shows only where a piece of wood behind a nest should show — a
              sliver at the left, tips above the rim, and the climb away to the right. The old far
              arm was drawn between the far rim and the near rim, which put it across the INSIDE of
              the bowl. It carries the same `cbBark` ramp as the bough (the gradient is
              `userSpaceOnUse` in each path's own group frame, so it turns with whatever references
              it) and one stop of shade over the top, because a limb behind another limb is in its
              shadow and not made of a different wood. */}
          <g transform={BOUGH3_XF}>
            <path d={BOUGH3_GEOM.body} fill="url(#cbBark)" fillRule="nonzero" />
            <path d={BOUGH3_GEOM.body} fill="#2A1A0A" fillRule="nonzero" opacity={0.22} />
          </g>
          <g transform={BOUGH2_XF}>
            <path d={BOUGH2_GEOM.body} fill="url(#cbBark)" fillRule="nonzero" />
            <path d={BOUGH2_GEOM.body} fill="#2A1A0A" fillRule="nonzero" opacity={0.52} />
          </g>
          {cradleBack.map((s) => (
          ...
          {/* 12b KNOTS, same slot and same reason: the bough is drawn over their butts, so what is
              left is four bumps in its own silhouette. */}
          {knots.map((s) => (
            <g key={s.seed} transform={`translate(${s.x} ${s.y}) rotate(${s.rot}) scale(${s.scale})`}>
              <path d={s.geom.body} fill="url(#cbBark)" fillRule="nonzero" />
            </g>
          ))}
          {endspray.map((s) => (
            <g key={s.seed} transform={`translate(${s.x} ${s.y}) rotate(${s.rot}) scale(${s.scale})`}>
          ...
          <g clipPath="url(#cbWood)" filter="url(#cbGrainSoft)">
            <g transform={BOUGH_XF}>
              {grain.map((g) => (
                <g
                  key={g.seed}
          ...
          <g clipPath="url(#cbWood)">
            <g filter="url(#cbContact)">
              <ellipse
                ...
                rx={620}
                ry={80}
                transform="rotate(-2 2270 1225)"
                fill="#2A1A0A"
                opacity={0.46}
              />
              <ellipse cx={1830} cy={1215} rx={260} ry={52} fill="#201306" opacity={0.2} />
              <ellipse cx={2760} cy={1210} rx={300} ry={56} fill="#201306" opacity={0.18} />
            </g>
          </g>
          ...
          {cradleFront.map((s) => (
            <g key={s.seed} transform={`translate(${s.x} ${s.y}) rotate(${s.rot}) scale(${s.scale})`}>
              <path
                d={s.geom.body}
                fill={`url(#${gradId(2, litUpAt(s.rot), bucketOfHalf(s.geom.baseHalf))})`}
                fillRule="nonzero"
              />
            </g>
          ))}
