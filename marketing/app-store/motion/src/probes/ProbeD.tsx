...
export const SCENE = {k: 0.55, cx: 2380, cy: 950};
...
/// came down to 41% of the safe width instead — see SCENE — and the lane is now the whole middle
/// band. With the word moved to the TOP of the safe box that band is now the lower two thirds of
/// it, and these are the three numbers the routing is solved against:
///
///   the word's ink       x 1152..1934.6, y 548..721                                   (ceiling)
///   the word's guard     x 1082..2004.6, y 478..791    the ink grown 70px             (ceiling)
///   the nest's ink top   772 at world x 2320-2440, 893 at 2040, absent left of 2027   (floor)
///   the safe box         x 1097..2742,  y 493..1152
///
/// The word moved to the top band, so this circuit was re-cut around it rather than nudged. Under
/// the word the bird now flies a DIPPED cruise — 949 at x 1170, 955 at 1450, 943 at 1730 — which is
/// 240px lower than the old flat lane at y 715 and puts its belly at y 1016, filling the bottom-left
/// corner the wordmark used to occupy. From x 1944 it pulls up at 62 degrees to clear the nest's
/// leading flank, levels at y 672 over the cup, releases, and leaves right and up as before.
///
/// The pull-up is steep because it has to be. The bird's ink must stay below y 791 while any part of
/// it is left of x 2005 (the guard) and above y 893 by the time any part of it reaches x 2027 (the
/// nest's fringe). That is a 130px climb inside a 22px window at the guard's bottom-right corner,
/// and every shallower routing tried put either the tail in the guard or the wing tip in the weave.
/// It reads as a flare rather than as a fault, which is what a bird arriving at a nest does anyway.
///
/// Waypoints 9-15 are untouched from the version before the word moved: the return leg already flew
/// the top bleed at y 178..330, which is 148px above the guard's own top edge, so a word in the top
/// band costs it nothing.
///
...
export const CIRCUIT: Way[] = [
  {x: -150, y: 800, s: 0.5}, // 0
  {x: 190, y: 850, s: 0.58}, // 1
  {x: 610, y: 925, s: 0.68}, // 2
  {x: 1170, y: 949, s: 0.7}, // 3
  {x: 1450, y: 955, s: 0.72}, // 4
  {x: 1730, y: 943, s: 0.74}, // 5
  {x: 1944, y: 917, s: 0.74}, // 6
  {x: 2144.6, y: 677, s: 0.76}, // 7
  {x: 2270, y: 672, s: 0.78}, // 8
  {x: 2530, y: 678, s: 0.76}, // 9
  {x: 2740, y: 620, s: 0.76}, // 10
  {x: 3080, y: 470, s: 0.62}, // 11
  {x: 3430, y: 295, s: 0.48}, // 12
  {x: 2900, y: 205, s: 0.42}, // 13
  {x: 1950, y: 178, s: 0.39}, // 14
  {x: 830, y: 330, s: 0.44}, // 15
];
const FLARE_WAY = 8;
...
export const U_REL = U_FLARE - 0.004;
...
export const poseAt = (u: number): BirdPose & {x: number; y: number; scale: number; heading: number; roll: number} => {
  const s = warpAt(u);
  const p = pathAt(s);
...
/// be seen against.
export const SETTLED = {x: 2192, y: 900, deg: 6};
...
/// 210, not 126. At 126 the cap height was 91px — 13.8% of the guaranteed-visible height — and the
/// name read as a caption on someone else's illustration. 210 puts the cap at 151px, 22.9% of the
/// safe height.
///
/// 210 is a CEILING, not a preference, and the bound is worth writing down because it is not
/// obvious. The nest's ink starts at screen x 1971 (from the audit's own nest box). The word has to
/// stay left of that by the same 70px the bird is held to, and its left edge cannot cross the safe
/// box at x 1097 or the store may crop the N. So the widest the word can be is 1901 - 1097 = 804px,
/// and at this face's 3.86:1 ink-width-to-size ratio that is size 208.
///
/// THE TOP BAND, and it is a measured result rather than a preference. The word was in the LOWER
/// left (baseline 1088, ink 916..1090) and the note back was that it sits too low: 55px of paper
/// above it inside the safe box and 423px of empty sky over that. It is now at the top of the
/// guaranteed view at very nearly the same size — ink 548..721, cap 169.2px by the model and 173px
/// measured off the render, 25.7%/26.3% of the safe height against 172.1/176 before.
///
/// The reason it was not there already is the bird, and that reason is real but it is a WIDTH
/// problem, not a height one. Word and nest do not stack: the word ends at x 1902 and the nest's
/// ink starts at 2027, so they sit side by side and the 659px of safe height is not the binding
/// budget. What binds is that the safe box is cut into two rooms with no legal door between them:
///
///   Band A   x 2005..2742, y 493..772   right of the word's guard, above the nest
///   Band B   x 1097..2027, y 791..1152   below the word's guard, left of the nest
///
/// and the guard's right edge is only 22px left of the first column the nest occupies (2027.2). A
/// bird under the word must have its ink BELOW y 791; 22px later it must have its ink ABOVE y 893,
/// which is where the nest's fringe starts. That door is the whole design problem, and it is why
/// the earlier 275pt centred attempt logged 34 consecutive onWORD frames.
///
/// Three things bought the door, in order of how much they were worth:
///   1. TRACKING 0.08 -> 0. Worth 56px of ink width at no cost in cap height at all, and every one
///      of those pixels becomes climb run for the bird. This is the single largest lever; the word
///      is a four-letter all-cap serif at 235pt and at display size the face's own fit is right
///      (measured inter-glyph ink gaps 9/15/15px, so nothing is colliding).
///   2. The bird's lane dropped 240px, from y ~715 to a dipped cruise at y 943..955, and the
///      approach became a 62-degree pull-up from x 1944 instead of a flat glide over the rim.
///   3. The bird's scale on that leg went 0.86 -> 0.74. Its wing tip on the upstroke reaches 118px
///      above the path point at 0.86 and 102px at 0.74, and that 16px is spent entirely on the
///      guard's bottom edge.
///
/// MEASURED, against the CORRECTED WORD_RATIO below: at size 235 the tightest frame in the loop
/// clears WORD_GUARD by 13.3px and the nest's ink by 14.6px. Size is a cliff, not a slope, because
/// the door between the two bands is only tens of pixels wide: 232 clears by 18.3, 225 by 28.4, and
/// 239 has no clean routing at all — its guard reaches x 2018 against a nest that starts at 2027, so
/// there is a 9px door and no bird fits through it. 235 is therefore the ceiling with a real margin.
///
/// The word cannot move DOWN either. Every pixel the ink descends pushes the guard's floor down and
/// the bird's lane with it, and the lane has nowhere to go: at ink top 578 the tightest slack is
/// 14.3px and by 608 there is no clean routing. The window is about 30px tall, so the top band is
/// not a choice among several — it is the only place a word this size fits at all.
///
...
/// it reads as placed rather than as cropped, and the ink top keeps the same 55px off y 493.
export const WORD = {left: 1152, baseline: 717.2, size: 235, track: 0};
/// MEASURED OFF A RENDER, and the number it replaces was wrong in two ways at once. The old
/// constant was `3.14 + 4 * track`. Both terms are wrong:
///
///   - the base is 3.330, not 3.14. Read off the rendered PNG with a luma threshold at size 239,
///     track 0: the ink runs x 1155..1948 against a text anchor at 1152, so the ink reaches
///     796/239 = 3.330 sizes past the anchor. 3.14 understates the word's ink by 45px at this size.
///   - the coefficient is 3, not 4. SVG does apply letterSpacing after every glyph including the
///     last, so the ADVANCE is four tracks wide — but the INK stops at the K, so only the three
///     internal gaps move it. Checked against the previous pass's own measurement at track 0.08
///     (ink 1155..2003, i.e. 851/239 = 3.561 = 3.330 + 3 x 0.08 to within a pixel).
///
/// This matters more than a tidy-up: WORD_GUARD is built on it, so for two passes the rectangle the
/// bird was tested against was 26-45px narrower than the type actually is, and a routing that the
/// audit called clean had the bird's tail 27px inside the true guard. The corrected number is what
/// the sizing below is solved against.
const WORD_RATIO = 3.33 + 3 * WORD.track;
export const WORD_RECT = {
  x0: WORD.left,
  y0: WORD.baseline - 0.72 * WORD.size,
  x1: WORD.left + WORD_RATIO * WORD.size,
  y1: WORD.baseline + 4,
};
...
export const nestInkTopWorld = (xWorld: number) => {
...
export const birdPoints = (u: number) => {
  const pose = poseAt(u);
...
export const boxOf = (pts: [number, number][]) => {
...
export const ProbeD: React.FC<{guides?: boolean}> = ({guides = false}) => {
  ...
  return (
    <AbsoluteFill style={{background: '#F3E3C4'}}>
      <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`}>
        ...
        {/* 26 the wordmark. TOP LEFT of the guaranteed view and off the nest's axis entirely — the
            type ends at world x 1934.6 and the nest's ink starts at 2027.2, so they never share a
            column and the two masses sit on a diagonal rather than stacked. 55px inside the safe
            box's left edge and 55px below its top edge, ink 548..721, cap 169.2px. The bird's
            dipped cruise runs 222px below the ink and its pull-up passes 83px to the right of the K.
            It is the last thing in the composition and nothing, including proximity, may enter its
            rectangle: the audit tests the bird against WORD_GUARD, the ink grown by 70px, and the
            tightest frame in the loop clears that rectangle by 13.3px. */}
        <text
          x={WORD.left}
          y={WORD.baseline}
        ...
        {DEBUG ? (
          <g fontFamily="monospace" fontSize={38} fill="#B00000">
            ...
            <text x={40} y={1620}>
              {`uFlare=${U_FLARE.toFixed(4)} rel=${REL_FRAME.toFixed(1)} land=${LAND_FRAME.toFixed(1)} pathLen=${PATH.total.toFixed(0)}`}
            </text>
          </g>
        ) : null}
